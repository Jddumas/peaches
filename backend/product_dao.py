...
class ProductDAO:
    ...
    def get_all(self):
        ...
        rows = self.db.query(SQL.PRODUCT.GET_ALL)
    ...
    def get(self, sku):
        row = self.db.query(SQL.PRODUCT.GET_BY_ID, {"sku": sku})
    ...
    def create(self, product_configs={}):
        ...
        row = self.db.update(SQL.PRODUCT.INSERT, product.dict())
    ...
    def update(self, sku, product_configs):
        ...
        self.db.update(SQL.PRODUCT.UPDATE, altered_product_configs)

        # Step 4: return altered product calculated above.
        return altered_product.dict()
